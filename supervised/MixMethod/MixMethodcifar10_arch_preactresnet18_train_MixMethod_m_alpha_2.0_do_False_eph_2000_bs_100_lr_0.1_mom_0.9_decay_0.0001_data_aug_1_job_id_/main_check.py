...
# cudnn에서 벤치 마크 모드를 활성하합니다. 벤치 마크 모드는 네트워크의 입력 크기가 다를 때마다 좋습니다.
# 이런 식으로 cudnn은 특정 구성에 대해 최적의 알고리즘 세트를 찾습니다.
# 이를 이용하면 일반적으로 runtime이 빨라집니다.
# 그러나 각 iteration에서 입력의 크기가 변경되면 cudnn은 새로운 크기가 나타날 때마다 
# 벤치마킹하여 런타임 성능을 저하시킬 수 있습니다.


# 그냥 이름을 만들어주는 함수, 내가준 옵션대로 이름을 만들어줌
...
def save_checkpoint(state, is_best, save_path, filename):
    ...
    if is_best:
        ...
        shutil.copyfile(filename, bestname) # filename의 파일을 bestname으로 copy해준다.

# +
# 여기는 learning rate scheduler부분
# gammafmf 이용하여 각 step 마다 learning rate를 조정해준다.
...
def accuracy(output, target, topk=(1,)): # output은 모델의 결과
...
#             print("reweighted_target shape : ", reweighted_target.shape)
            ...
            """
        elif args.train== 'MyMixup':
            input_var, target_var = Variable(input), Variable(target)
            MyMixup_num_layer = args.MyMixup_num_layer
            mixup_out, out = model(input_var, target_var, mixup_hidden= True, mixup_alpha = args.mixup_alpha,MyMixup = True, MyMixup_num_layer = MyMixup_num_layer)

                
#             print("=" * 100 + "Mixup out")
#             print(mixup_out.requires_grad)
#             print("=" * 100 + "out")
...
#            assert False,"STOP@@@@@@@@@@@@@@@@@"
        ...
        if args.train == 'MyMixup':
            prec1, prec5 = torch.tensor([0.,0.]).to("cuda")
#            prec1, prec5 = accuracy(mixup_out, out, topk=(1, 5))
        else:
...
def validate(val_loader, model, log, args):
  losses = AverageMeter()
  top1 = AverageMeter()
  top5 = AverageMeter()
  ...
  for i, (input, target) in enumerate(val_loader):
    ...
    # compute output
    if (args != None) and args.validate_mix:
        output, reweighted_target = model(input_var,target_var, mixup= True, mixup_alpha = args.mixup_alpha)
        loss = bce_loss(softmax(output), reweighted_target)#mixup_criterion(target_a, target_b, lam)    
    else:
        output = model(input_var)
...
def main():
    ...
    # 내가 준 옵션대로 exp_name을 만들어줌, exp_name은 string형
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            distant =  args.epochs - recorder.total_epoch
            recorder.total_epoch += distant
            print(recorder.total_epoch)
            print("="*200)
            recorder.epoch_losses = np.append(recorder.epoch_losses, np.zeros((distant, 2), dtype=np.float32), axis=0)
            recorder.epoch_accuracy = np.append(recorder.epoch_accuracy, np.zeros((distant, 2), dtype=np.float32), axis=0)
    ...
    #evaluate를 하고 시작할지, 경우에 따라 처음에 evaluate를 하는게 좋은 경우가 있음
    if args.evaluate:
        validate(test_loader, net, log, args)
        print("==============================================EVALUATE======================================================")
#        return
...
#                 args.train = "MyMixup"
        ...
        current_learning_rate = adjust_learning_rate(optimizer, epoch, args.gammas, args.schedule) # learning rate를 수정해줌
